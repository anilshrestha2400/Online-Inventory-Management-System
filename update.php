 ...
 <body>
	...
	?>
 	<div class="col-lg-6 m-auto">
		...
		Product image: <input type="file" name="image" style="height: 45px;" value="<?php echo $erp;?>" required ><br>
		Description: <input type="text" name="text" style="height: 75px;" value="<?php echo $ert;?>" required><br>
		Product name: <input type="text" name="text1" style="height: 40px;" value="<?php echo $erx;?>" required><br>
		Product price: <input type="number"  name="text2" style="height: 40px;" value="<?php echo $erz;?>" required><br>
	...
	if(isset($_POST['btn'])){
		...
		if(empty($image)){
			$erp ="product image is not added";
		}
		if(empty($text)){
			$ert ="description is not added";
		}
		if(empty($text1)){
			$erx ="product name is not added";
		}
		if(empty($text2)){
			$erz ="product price is not added";
		}

		$conn = new mysqli("localhost","root","","inventory");
		if($image && $text && $text1 && $text2){
			$sql="update dash set id=$id, product_image='$image', description='$text', product_name='$text1', product_price='$text2' where id=$id";
